...
export const makeRevision = async (req, res) => {
    ...
    try {
        ...
        if (!userId || !user) {
            return res
                .status(401)
                .json({ message: "Unauthorized access. Please sign in." });
        }
        if (user.credits < 5) {
            return res.status(403).json({
                message: "Insufficient credits. Please add more credits to make changes.",
            });
        }
        if (!message || message.trim() === "") {
            return res.status(400).json({
                message: "Please enter a valid prompt or description for your changes.",
            });
        }
        ...
        if (!code) {
            ...
            return res.status(500).json({
                message: "Unable to generate the code. Please try again or contact support if the issue persists.",
            });
        }
